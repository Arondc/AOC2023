...
class Day10 {
    ...
    data class Maze(val pipes: Set<Pipe>) {
        ...
        fun findConnectedPipes(pipe: Pipe): Set<Pipe> =
            pipe.connections.map { c ->
                when (c) {
                    Connection.NORTH -> Pair(0, -1)
                    Connection.SOUTH -> Pair(0, 1)
                    Connection.EAST -> Pair(1, 0)
                    Connection.WEST -> Pair(-1,0)
                }
            }.mapNotNull { getPipeAt(pipe.column + it.first, pipe.row + it.second) }
                .filter {neighbourPipe -> pipe.isConnectedTo(neighbourPipe)}.toSet()
    }

    data class Pipe(val column: Int, val row: Int, val char: Char) {
        val connections: Set<Connection> = when (char) {
            '.' -> setOf()
            '|' -> setOf(Connection.NORTH, Connection.SOUTH)
            '-' -> setOf(Connection.EAST, Connection.WEST)
            'L' -> setOf(Connection.NORTH, Connection.EAST)
            'J' -> setOf(Connection.WEST, Connection.NORTH)
            '7' -> setOf(Connection.WEST, Connection.SOUTH)
            'F' -> setOf(Connection.EAST, Connection.SOUTH)
            'S' -> setOf(Connection.NORTH, Connection.EAST, Connection.SOUTH, Connection.WEST)
            else -> throw RuntimeException("This is bad")
        }
            get

        fun isConnectedTo(other: Pipe): Boolean {
            for (connection in connections){
                val connected = when(connection){
                    Connection.NORTH -> other.connections.contains(Connection.SOUTH) && this.row-1 == other.row
                    Connection.SOUTH -> other.connections.contains(Connection.NORTH)&& this.row+1 == other.row
                    Connection.WEST -> other.connections.contains(Connection.EAST)&& this.column-1 == other.column
                    Connection.EAST -> other.connections.contains(Connection.WEST) && this.column+1 == other.column
                }
    ...
    class FourWayFlood(val maze: Maze) {
        data class FloodedPipe(val pipe: Pipe, val liquidNr : Int)

        private val floodedPipes = mutableSetOf<FloodedPipe>()
        private val startPipe = maze.start

        fun flood() {
            floodedPipes.add(FloodedPipe(startPipe,0))
            val floodStarterPipes =
                maze.findConnectedPipes(startPipe).mapIndexed { index, pipe -> FloodedPipe(pipe, index + 1) }
                    .onEach { floodedPipes.add(it) }.toSet()

            var newlyFloodedPipe = floodStarterPipes
            do {
                newlyFloodedPipe =
                    newlyFloodedPipe.mapNotNull { p ->
                            val notFloodedNeighbors = maze.findConnectedPipes(p.pipe) - floodedPipes.map { it.pipe }.toSet()
                            if (notFloodedNeighbors.isEmpty()) {
                                null
                            }else {
                                FloodedPipe(notFloodedNeighbors.single(),p.liquidNr)}
                        }
                        .onEach { fp -> floodedPipes.add(fp) }.toSet()
            }while (newlyFloodedPipe.isNotEmpty())
            println(floodedPipes.groupingBy { it.liquidNr }.eachCount())
        }

    }

    enum class Connection {NORTH, EAST, SOUTH, WEST}


    fun star1() {
        val maze = parseMaze(day10input)
        println(maze)
        val flood= FourWayFlood(maze)
        flood.flood()

    }

    private fun parseMaze(input: String): Maze {
        val allPipes = mutableSetOf<Pipe>()

        input.lines().withIndex().forEach { (rowIdx, row) ->
            row.forEachIndexed { colIdx, char ->
                allPipes.add(Pipe(colIdx, rowIdx, char))
            }
        }
        return Maze(allPipes)
    }
